...
Provide clear, structured answers with bold text.
`;
...
`;
    ...
    if (parsedData.recommendations && Array.isArray(parsedData.recommendations)) {
      parsedData.recommendations = parsedData.recommendations.map((rec: any, idx: number) => ({
    ...
    res.json({ recommendations: stadiumState.recommendations, error: error.message });
  }
});

// Fallback generator for offline/local simulation
function simulateCopilotResponse(message: string, res: any) {
  const query = message.toLowerCase();
  let responseText = "";

  if (query.includes("gate") || query.includes("crowd") || query.includes("congestion")) {
    responseText = `
**Crowd Control Analysis (Local Simulator)**:
- **Gate B** is currently highly congested at **92% density** with an estimated queue time of **38 minutes**.
- This queue is caused by an active incident: **Turnstile Fault - Gate B** (logged at 10:35).
- **Suggested Action**: 
  1. Redirect outbound flows from Gate B towards **Gate D** (density 15%, queue 2 mins).
  2. Dispatch 4 volunteers with handheld scanning devices to clear the Gate B turnstile queue manually.
- **Expected Impact**: Reducing Gate B wait time by **35%** within 10 minutes.
`;
  } else if (query.includes("incident") || query.includes("medical") || query.includes("heat")) {
    responseText = `
**Incident Intelligence Summary**:
There are currently **3 active/dispatched incidents**:
1. **Turnstile Fault - Gate B** (TECH, High Priority): Scanners offline. Queue building.
2. **Debris on Access Ramp** (FACILITY, Medium Priority): 2 volunteers already dispatched, currently clearing.
3. **Heat Exhaustion Support** (MEDICAL, High Priority, Section 114): A fan needs cooling and first aid.
**Copilot Recommendation**: Call the on-duty first aid squad to Section 114 immediately, and assign 2 standby volunteers to assist.
`;
  } else if (query.includes("transit") || query.includes("transport") || query.includes("metro")) {
    responseText = `
**Transportation Optimization Advisor**:
- **Olympic Park Metro Line 1** is **Congested** with a **20-minute waiting time**.
- **Official Rideshare Gate D** is **Delayed** with a **15-minute wait** due to perimeter traffic.
- **Tournament Shuttle Bus** is running **Normal** with 5-minute wait times.
- **Recommendation**: Update spectator display monitors in the main concourse to recommend the Tournament Shuttle Bus for downtown transfers to bypass the Metro queue.
`;
  } else if (query.includes("sustainability") || query.includes("recycle") || query.includes("solar")) {
    responseText = `
**Sustainability Insights Report**:
- Stadium solar array output: **840 kW** green energy generated today.
- Recycled waste collected: **4,210 kg** (target: 5,000 kg for matchday).
- Water saved: **12,450 litres** via smart greywater recycling.
- **AI Recommendation**: Broadcast stadium waste recycling challenges on big screens during half-time to encourage fans to use green bins.
`;
  } else if (query.includes("spanish") || query.includes("translate") || query.includes("multilingual")) {
    responseText = `
**Multilingual Assistance Desk**:
Here is the translation of the emergency evacuation detour warning to **Spanish** for Gate B:
*"Atención espectadores: Gate B está temporalmente congestionada. Por favor, desvíense hacia Gate D (Entrada VIP Oeste) para una salida rápida. Sigan las indicaciones de los voluntarios."*
This announcement can be pushed to dynamic signage in Section 114-120 immediately.
`;
  } else {
    responseText = `
Hello! I am your **FIFA 2026 Operations Copilot** (Simulated Mode).
I can assist with real-time stadium intelligence:
1. **Crowd Density** checks and bottleneck mitigations.
2. **Volunteer dispatching** and triage for active incidents.
3. **Transit and dynamic detour** optimizations.
4. **Sustainability statistics** and energy reporting.
5. **Multilingual translation** for announcements.

Ask me anything about Gate B congestion, current incidents, or transit updates!
`;
  }
